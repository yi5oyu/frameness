...
Inherited context is read-only background. Do not edit code, write files, mutate `.agents/` state, modify `.agents/state.json`, run formatters, invoke workflow handoffs, or implement anything. Use only inherited context, the prompt-safe initial idea, locked topology, current scores/gaps, established facts, prior decisions, and read-only repo/context inspection if available.
